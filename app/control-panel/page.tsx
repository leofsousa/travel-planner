...
// 🔥 FUNÇÃO PARA CALCULAR O CUSTO TOTAL (HOTEL + CARRO)
function calculateTotalCost(request: any): number {
  let total = 0;

  // Custo do hotel
  if (request.hotel_planning?.[0]?.rooms) {
    const rooms = request.hotel_planning[0].rooms || [];
    rooms.forEach((room: any) => {
      ...
      periods.forEach((period: any) => {
        ...
        total += days * period.dailyRate;
      });
    });
  }

  // 🔥 Custo do carro
  if (request.request_cars) {
    // Pega o primeiro item se for array
    const carData = Array.isArray(request.request_cars) 
      ? request.request_cars[0] 
      : request.request_cars;
    
    if (carData?.car_rentals) {
      // car_rentals pode ser array ou objeto
      const rentals = Array.isArray(carData.car_rentals) 
        ? carData.car_rentals 
        : [carData.car_rentals];
      ...
      rentals.forEach((rental: any) => {
        ...
        } else if (rental.daily_rate && rental.start_date && rental.end_date) {
          const startParts = rental.start_date.split("-").map(Number);
          const endParts = rental.end_date.split("-").map(Number);
          const start = new Date(startParts[0], startParts[1] - 1, startParts[2]);
          const end = new Date(endParts[0], endParts[1] - 1, endParts[2]);
          const days = Math.ceil((end.getTime() - start.getTime()) / (1000 * 60 * 60 * 24));
...
function calculateMetrics(requests: Request[]) {
  ...
  requests.forEach((r) => {
    ...
    // 🔥 CORREÇÃO: Contagem de carros
    if (r.request_cars) {
      const carData = Array.isArray(r.request_cars) 
        ? r.request_cars[0] 
        : r.request_cars;
      ...
      if (carData?.car_rentals) {
        const rentals = Array.isArray(carData.car_rentals) 
          ? carData.car_rentals 
          : [carData.car_rentals];
        totalCarRentals += rentals.length;
      }
    }
